      ...
      <S.BoxContainer height={height}>
        <S.BarContainer height={height}>
          <S.PosessionBox height={height} bgColor="#0046FF">
            ...
            <Text style={{ color: "white" }}>{[props.asset.usableAsset].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
          </S.PosessionBox>
          <S.PosessionBox height={height} bgColor="#6E67E7">
            <Text style={{ color: "white" }}>생활비</Text>
            <Text style={{ color: "white" }}>{[props.asset.livingCost].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
          </S.PosessionBox>
          <S.PosessionBox height={height} bgColor="#F96464">
            <Text style={{ color: "white" }}>고정지출+월세</Text>
            <Text style={{ color: "white" }}>
              {[props.asset.fixedCost.totalFixedCost].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
            </Text>
          </S.PosessionBox>
        </S.BarContainer>
        <SmallButton
